...
class SafetyVisitor : public clang::RecursiveASTVisitor<SafetyVisitor> {
  private:
    ...
    bool mayThrow;
  public:
      explicit SafetyVisitor(clang::ASTContext &ctx) : Context(ctx), mayThrow(false) {}
      ...
      bool VisitCallExpr(clang::CallExpr *call) {
          ...
          const clang::FunctionProtoType *proto = callee->getType()->getAs<clang::FunctionProtoType>();
          if (!proto) {
              mayThrow = true;
              return false;
          }
          ...
          bool isNoexcept = (spec == clang::EST_BasicNoexcept || spec == clang::EST_NoThrow);
          if (!isNoexcept) {
              mayThrow = true;
              return false;
          }
...
// Функция для создания нового FunctionType с noexcept
clang::QualType makeNoexceptType(clang::ASTContext &ctx, const clang::FunctionProtoType *proto) {
    clang::FunctionProtoType::ExtProtoInfo info = proto->getExtProtoInfo();
    info.ExceptionSpec.Type = clang::EST_BasicNoexcept;

    return ctx.getFunctionType(proto->getReturnType(), proto->getParamTypes(), info);
}
...
public:
    ...
    bool VisitFunctionDecl(clang::FunctionDecl *FD) {
        ...
        const clang::FunctionProtoType *proto = FD->getType()->getAs<clang::FunctionProtoType>();
        if (!proto)
            return true;
