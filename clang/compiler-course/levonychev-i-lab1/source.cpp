...
public:
  ...
  bool VisitUnaryOperator(UnaryOperator *Node) {
    if (!Node->isIncrementDecrementOp()) return true;
    ...
    if (auto *UO = dyn_cast<UnaryOperator>(Sub)) {
      if (UO->getOpcode() == UO_Deref) {
        if (auto *DRE = dyn_cast<DeclRefExpr>(UO->getSubExpr()->IgnoreParenImpCasts())) {
          if (DRE->getDecl() == Target) DataMutated = true;
        }
      }
    }
    return true;
...
public:
  void HandleTranslationUnit(ASTContext &Context) override {
    ...
    const llvm::RewriteBuffer *RewriteBuf = TheRewriter.getRewriteBufferFor(Context.getSourceManager().getMainFileID());
    if (RewriteBuf) {
...
static FrontendPluginRegistry::Add<ConstAnalysisAction>
    X("const_change_plugin", "Suggests const for pointers and references based on mutations");