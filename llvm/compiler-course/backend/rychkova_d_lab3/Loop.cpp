...
private:
  ...
  bool performUnrolling(MachineLoop *L, MachineFunction &MF, 
                        int UnrollFactor, MachineLoopInfo &MLI);

  void cloneLoopBody(MachineLoop *L, MachineFunction &MF, 
...
X("loop-unroll-limited", "Loop unroll pass (max 5 iterations)", false, false);
...
bool LoopUnrollPass::performUnrolling(MachineLoop *L, MachineFunction &MF,
  ...
  SmallVector<MachineBasicBlock *, 8> LoopBlocks(L->block_begin(), L->block_end());
...
namespace llvm {
  FunctionPass *createLoopUnrollLimitedPass() {
    return new LoopUnrollPass();
  }
}