...
class MissingOverrideVisitor final : public clang::RecursiveASTVisitor<MissingOverrideVisitor> {
public:
  explicit MissingOverrideVisitor(clang::ASTContext *context) : m_context(context) {}

  bool VisitCXXMethodDecl(clang::CXXMethodDecl *method_decl) {
    if (method_decl->isImplicit()) return true;
    if (!method_decl->getParent()) return true;

    if (m_context->getSourceManager().isInSystemHeader(method_decl->getLocation()))
      return true;

    if (method_decl->size_overridden_methods() == 0) return true;
    
    if (method_decl->hasAttr<clang::OverrideAttr>()) return true;

    clang::DiagnosticsEngine &diag_engine = m_context->getDiagnostics();

    unsigned main_diag_id = diag_engine.getCustomDiagID(
        clang::DiagnosticsEngine::Warning,
        "method '%0' overrides a virtual function but is missing the 'override' specifier");
...
public:
  explicit MissingOverrideConsumer(clang::ASTContext *context) : m_visitor(context) {}
...
static clang::FrontendPluginRegistry::Add<MissingOverrideAction>
    X("dolov_v_lab1", "A professional plugin to enforce 'override' specifiers usage");